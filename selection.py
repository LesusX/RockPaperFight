...
# TODO: The highlights lag. Convert all images properly and then clean the unnecessary button loops
btns = [button1, button2, button3, button4, button5, button6, button7, button8, button9, button10, button11]
shown_champion = [MeleeChampionOne] # Start the selection screen with a chmpion preselected 
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if DASH_BUTTON.checkForInput(MENU_MOUSE_POS):
                    ...
                    shown_champion.append(MeleeChampionOne)
                    for btn in btns:
                        if btn != button1:
                            btn.is_locked = False 
                ...
                if CASANDRA_BUTTON.checkForInput(MENU_MOUSE_POS):
                    ...
                    shown_champion.clear()
                    shown_champion.append(MeleeChampionTwo)
                    for btn in btns:
                        if btn != button2:
                            btn.is_locked = False 
                ...
                if IVES_BUTTON.checkForInput(MENU_MOUSE_POS):
                    ...
                    shown_champion.clear()
                    shown_champion.append(MeleeChampionThree)
                    for btn in btns:
                        if btn != button3:
                            btn.is_locked = False  
                    
                if NORWIN_BUTTON.checkForInput(MENU_MOUSE_POS):
                    champ_selector.champion_selection(MeleeChampionFour)
                    shown_champion.clear()
                    shown_champion.append(MeleeChampionFour)
                    for btn in btns:
                        if btn != button4:
                            btn.is_locked = False 

                if HASSARON_BUTTON.checkForInput(MENU_MOUSE_POS):
                    champ_selector.champion_selection(MeleeChampionFive)
                    shown_champion.clear()
                    shown_champion.append(MeleeChampionFive)
                    for btn in btns:
                        if btn != button5:
                            btn.is_locked = False 

                if IRATHAS_BUTTON.checkForInput(MENU_MOUSE_POS):
                    champ_selector.champion_selection(MeleeChampionSix)
                    shown_champion.clear()
                    shown_champion.append(MeleeChampionSix)
                    for btn in btns:
                        if btn != button6:
                            btn.is_locked = False 

                if IRGOMIR_BUTTON.checkForInput(MENU_MOUSE_POS):
                    champ_selector.champion_selection(MeleeChampionSeven)
                    shown_champion.clear()
                    shown_champion.append(MeleeChampionSeven)
                    for btn in btns:
                        if btn != button7:
                            btn.is_locked = False 

                if LAMBERT_BUTTON.checkForInput(MENU_MOUSE_POS):
                    champ_selector.champion_selection(MeleeChampionEight)
                    shown_champion.clear()
                    shown_champion.append(MeleeChampionEight)
                    for btn in btns:
                        if btn != button8:
                            btn.is_locked = False 

                if NIMBUS_BUTTON.checkForInput(MENU_MOUSE_POS):
                    champ_selector.champion_selection(MeleeChampionNine)
                    shown_champion.clear()
                    shown_champion.append(MeleeChampionNine)
                    for btn in btns:
                        if btn != button9:
                            btn.is_locked = False 

                if NOBURO_BUTTON.checkForInput(MENU_MOUSE_POS):
                    champ_selector.champion_selection(MeleeChampionTen)
                    shown_champion.clear()
                    shown_champion.append(MeleeChampionTen)
                    for btn in btns:
                        if btn != button10:
                            btn.is_locked = False 

                if YAHIRO_BUTTON.checkForInput(MENU_MOUSE_POS):
                    champ_selector.champion_selection(MeleeChampionEleven)
                    shown_champion.clear()
                    shown_champion.append(MeleeChampionEleven)
                    for btn in btns:
                        if btn != button11:
                            btn.is_locked = False 
