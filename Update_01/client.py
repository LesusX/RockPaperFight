...
BG = pygame.image.load("assets/menu_pic.png")
...
def play_offline():
    while True:
        ...
        PLAY_BACK = BaseButton(image=None, pos=(640, 390), 
                            text_input="Play ofline", font=get_font(50), base_color="White", hovering_color="Green")

        ON_BACK = BaseButton(image=None, pos=(640, 480), 
                            text_input="Play online", font=get_font(50), base_color="White", hovering_color="Green")

        SC_BACK = BaseButton(image=None, pos=(640, 560), 
                            text_input="BACK to main menu", font=get_font(50), base_color="White", hovering_color="Green")
        
        
        PLAY_BACK.changeColor(PLAY_MOUSE_POS)
        PLAY_BACK.update(win)
        SC_BACK.changeColor(PLAY_MOUSE_POS)
        SC_BACK.update(win)
        ON_BACK.changeColor(PLAY_MOUSE_POS)
        ON_BACK.update(win)        
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if PLAY_BACK.checkForInput(PLAY_MOUSE_POS):
                    test.gameLoop()
            if event.type == pygame.MOUSEBUTTONDOWN:
                if ON_BACK.checkForInput(PLAY_MOUSE_POS):
                    play_online_win() # TODO: replace with main()
            if event.type == pygame.MOUSEBUTTONDOWN:
                if SC_BACK.checkForInput(PLAY_MOUSE_POS):
                    return False
        pygame.display.update()
